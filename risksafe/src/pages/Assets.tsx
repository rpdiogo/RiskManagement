...
import { Plus, Pencil, Trash2, Search, Server, Monitor, Wifi, Cloud } from 'lucide-react'
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {filtered.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-10 text-center text-slate-400 text-sm">Nenhum ativo encontrado</td></tr>
            )}
            {filtered.map(a => {
              ...
              const linkedRisks = a.riskIds ? a.riskIds.split(',').filter(Boolean) : []
              return (
                <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1">
                      {linkedRisks.length === 0 && <span className="text-slate-300 text-xs">—</span>}
                      {linkedRisks.map(rid => (
